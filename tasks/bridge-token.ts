...
task("bridge-token", "")
	...
	.setAction(async (args, hre: any) => {
		...
		await (await subfBridge.connect(signer).approve(signer.address, ethers.parseEther(args.amount))).wait();

		await subfBridge.connect(signer).bridge.estimateGas(args.dest, wallet, ethers.parseEther(args.amount));
		await (await subfBridge.connect(signer).bridge(args.dest, wallet, ethers.parseEther(args.amount))).wait();
